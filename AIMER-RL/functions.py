...
evaluate_path = "samples/evaluate_set/"
mutation_path = "samples/mutation/"

LIEF_EXCEPTIONS = (lief.lief_errors.file_error, lief.lief_errors.file_format_error)
...
def build_bytes(input_bytes, total_number_perturbations):
	...
	try:
		...
		eval_mod_file = mutation_path + str(rl.root_name) + str(total_number_perturbations)+'_m.exe'
		builder.write(eval_mod_file)
...
def rec_mod_files(input_bytes, actions, chosen_actions, inject_perturbation, total_number_perturbations, current_malware_name_var):
	EXCEPTIONS = (MemoryError, lief.lief_errors.file_error, lief.lief_errors.file_format_error, lief.lief_errors.not_found)
	'''
	...
	for idx in range(total_number_perturbations):
		try:
			malman = m.MalwareManipulator(current_bytes)
			next_action = actions[chosen_actions[idx]]
			# print(next_action)
			#Áp dụng action lên file
			inject_action = malman.__getattribute__(next_action)
			current_bytes = inject_action(current_bytes)

		except EXCEPTIONS as e:
			print('When injecting perturbation returned the error: ', e)
			return None

	return build_bytes(current_bytes, total_number_perturbations)###
